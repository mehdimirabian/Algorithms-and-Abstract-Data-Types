 ...
 * CruzID: mmirabia
 * File Name: List.c
 * File Purpose: ADT for list operations
 *
 */
...
void moveprevious(List L){
  if (L == NULL) {
    printf("List.c : 161: passing null ptr to moveprevious");
    return;
  }
...
List copyList(List L){
  ...
  List copy = newList();
  for(tmp = L->front; tmp != NULL; tmp = tmp->next){
...
List concat(List A, List B){
  ...
  for(tmp= B->front; tmp != NULL; tmp = tmp->next){
